...
scores. The result is a route cache consumed by the maintained Non-Oracle
pipeline (``prosys_shared.route_cache.load_route_records_from_cache``).
...
with cwd=stage1_retrosynthesis/). Aggregation reuses
``stage1_retrosynthesis/utils/get_ranked_topk.py``.
"""
...
def resolve_checkpoint(repo_root: Path, family: str, override: str | None) -> Path:
    ...
    finetune_root = repo_root / 'stage1_retrosynthesis' / 'results' / 'family_finetune' / dataset_name(family)
...
def main() -> None:
    ...
    output_dir = output_dir.resolve()  # subprocess runs with cwd=stage1_retrosynthesis/, so paths must be absolute
