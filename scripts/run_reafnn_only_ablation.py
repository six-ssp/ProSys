...
def main():
    ...
    for seed in seeds:
        for family in families:
            ...
            if record.exists():
                ...
                assert (dest / "candidate_audit.json").exists()
            ...
            command = [sys.executable, str(Path(__file__).resolve()), "--child",
                ...
                "--output_root", str(job), "--route_root", str(ROOT / "outputs/stage1_routes"),
                ...
                "--reafnn_post_fusion_validation_route_root", str(ROOT / "outputs/stage1_routes_validation")]
            start = time.time()
            ...
            result["input_sha256"] = {
                **{split: sha(split_file_for_family(ROOT, family, split)) for split in ("train", "val", "test")},
                "stage1_test": sha(ROOT / "outputs/stage1_routes" / family / "route_cache.json"),
                "stage1_validation": sha(ROOT / "outputs/stage1_routes_validation" / family / "route_cache.json"),
            }
    ...
    reporting._write_report(output_root=output, rows=rows, families=families, seeds=seeds,
        mainline_reference=reporting._load_mainline_reference(ROOT / "Experiment/stage23_parallel_post_fusion_multiseed_20260903/macro_mean_std.csv"),
        mainline_compact_root=ROOT / "Experiment/stage23_parallel_post_fusion_multiseed_20260903/compact/prosys")
