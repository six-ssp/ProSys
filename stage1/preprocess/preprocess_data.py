...
if __name__ == '__main__':
    def determine_dataset_splits(args, datadir):
        if args.test_only:
            requested = ['test']
            ...
            requested = ['test', 'val', 'train']
        elif args.splits != 'auto':
            requested = [split.strip() for split in args.splits.split(',') if split.strip()]
        else:
            requested = ['test', 'val', 'train']

        if args.splits == 'auto' and not any([
            args.test_only,
            args.train_only,
            args.test_except,
            args.train_except,
            ...
            requested = [
                split for split in ['test', 'val', 'train']
                if os.path.exists(os.path.join(datadir, f'raw_{split}.csv'))
            ]
            if not requested:
                requested = [
    ...
    parser.add_argument(
        ...
        type=str,
        default='auto',
        help='comma-separated splits to preprocess, e.g. train,val or train,val,test; auto uses available raw_*.csv files',
    )
    args = parser.parse_args()
    print('preprocessing dataset {}...'.format(args.dataset))
    print(args)
    dataset_roots = []
    env_dataset_root = os.environ.get('EDITRETRO_DATASETS_ROOT')
    if env_dataset_root:
        dataset_roots.append(Path(env_dataset_root))
    dataset_roots.extend([
        Path('../datasets'),
        Path('../../data/editretro/datasets'),
    ])

    dataset_root = None
    datadir = None
    for root in dataset_roots:
        candidate = root / args.dataset / 'raw'
        if candidate.is_dir():
            dataset_root = root
            datadir = candidate
            break
    ...
    if datadir is None:
        datadir = dataset_roots[0] / args.dataset / 'raw'
        raise FileNotFoundError(f'raw dataset directory not found: {datadir}')
    datadir = str(datadir)

    datasets = determine_dataset_splits(args, datadir)
    if not datasets:
        raise ValueError(f'no available splits found under {datadir}')

    random.seed(args.seed)

    if args.spe:
        savedir = str(dataset_root / args.dataset / f'aug{args.augmentation}')
    elif args.self:
        savedir = str(dataset_root / args.dataset / f'aug{args.augmentation}_self')
    else:
        savedir = str(dataset_root / args.dataset / f'aug{args.augmentation}_token')
    ...
    for i, data_set in enumerate(datasets):
        ...
        if 'mapped_reaction_smiles' in csv.columns:
        ...
        if args.samples > 0:
            ...
            reaction_list = list(csv[reaction_column])
        
        if args.validastrain and data_set == "train":
            csv_path = f"{datadir}/raw_val.csv"
            csv = pd.read_csv(csv_path)
            if reaction_column in csv.columns:
                reaction_list += list(csv[reaction_column])
            else:
