...
This is the completed, corrected ablation run for the current six-family ProSys mainline:

```text
target product
  -> family-tuned EditRetro route proposals
  -> widened KNN retrieval + ReaFNN condition-pool selection
  -> Reaction-GNN route features + XGBoost reranking and temperature regression
```
...
The independent audit passed for all six families and all six model variants: fixed manifest, candidate partition, 20-context cap, XGBoost target exclusion, GNN feature dimensions, and equality between the reported full reference and the rebuilt mainline.
...
All three arms use the same fine-tuned Stage 1 routes, the same 20-context cap, a frozen family Reaction-GNN encoder, and a separately trained XGBoost ranker and temperature regressor. The table shows pool coverage and end-to-end Sys@10.

| Family | Top-20 frequency Cover / Sys@10 | KNN + ReaFNN Cover / Sys@10 | KNN only Cover / Sys@10 |
|---|---:|---:|---:|
...
The route-conditioned KNN pool is decisively better than a global frequency prior. Relative to Top-20 frequency, the full pool gains `+23.54 pp` coverage and `+24.96 pp` Sys@10; KNN-only gains `+28.42 pp` coverage and `+25.61 pp` Sys@10. This supports the central Stage 2 claim: local precedent retrieval, not a global condition prior, is what makes the feasible candidate pool useful.

The ReaFNN expansion is not a stable macro-average Sys@k improvement over raw KNN in this run: full pool Sys@10 is `42.68%` versus `43.33%` for KNN-only. It helps Friedel-Crafts acylation and alkylation, is tied for Beckmann, and is lower on the other three families. It should therefore be described as a controlled candidate-composition mechanism, not as a universally improving ranking component. Conditional temperature quality is better in the full arm (`11.28 C` MAE and `61.60%` within `+/-10 C`) than KNN-only (`13.62 C`, `54.70%`), but the supports differ (`1,603` vs `1,795`) because temperature is evaluated only on exact retrieved systems.
...
| Family | Full Sys@10 | Without Reaction-GNN Sys@10 | Without XGBoost Sys@10 |
|---|---:|---:|---:|
...
Removing XGBoost reduces macro Sys@1 from `27.64%` to `12.30%` and Sys@10 from `42.68%` to `31.31%` (`-11.37 pp`). Every family loses Sys@10 without learned reranking. This is the cleanest Stage 3 conclusion: KNN/ReaFNN make the correct system available, while XGBoost is needed to move it near the top of the recommendation list.

The Reaction-GNN feature block has a mixed ranking effect: removing it increases macro Sys@10 by `1.08 pp` (`43.76%` versus `42.68%`). It is better on three families and lower on three families. It does, however, improve the conditional temperature metrics on the same `n = 1,603` exact-system support: MAE `11.28 C` versus `12.51 C`, and `+/-10 C` hit rate `61.60%` versus `58.30%`. The correct claim is therefore that graph features are an auxiliary representation with a temperature-regression benefit in this run, not a demonstrated universal Sys@k gain.

## A4: ReaFNN-Graph Interaction Control

| ReaFNN pool | Reaction-GNN features | Macro Sys@1 | Macro Sys@10 | MRR | Temp MAE | Temp +/-10 C |
|---|---|---:|---:|---:|---:|---:|
...
The factorial control confirms that neither ReaFNN nor the Reaction-GNN block has a simple, additive macro Sys@10 contribution. ReaFNN changes from `-0.65 pp` with graph features to `+0.81 pp` without them; graph features change from `-1.08 pp` with ReaFNN to `+0.38 pp` without it. The modules interact, so a single-arm comparison would overstate their individual effect. The full configuration provides the best macro temperature MAE and `+/-10 C` hit rate, while the no-graph ReaFNN arm gives the highest macro Sys@10 in this specific run.

## Temperature Result

For the full mainline, temperature is evaluated only when the highest-ranked prediction is an exact route-and-condition match with a valid gold temperature. Across the six families this gives `n = 1,603`, MAE `11.28 C`, and hit rates of `37.96%`, `61.60%`, and `82.98%` within `+/-5 C`, `+/-10 C`, and `+/-20 C`, respectively. These are conditional regression results and must not be interpreted as temperature accuracy over all 3,860 test products.
...
2. Local KNN retrieval is strongly supported: it substantially outperforms a size-matched global frequency pool in coverage and all reported Sys@k metrics.
3. XGBoost reranking is strongly supported: removing it reduces Sys@10 for every family and lowers macro Sys@10 by `11.37 pp`.
4. ReaFNN and Reaction-GNN should not be claimed as universal Sys@k improvements from this run. Their controlled effects are mixed, while the full configuration has the strongest conditional temperature metrics.
