"""Shared feature helpers for the maintained ProSys pipeline."""
...
def reaction_morgan_fp(reactants: str, product: str, fpsize: int = 4096, radius: int = 2) -> np.ndarray:
    ...
    if reactant_mol is None or product_mol is None:
        # Stage 1 can predict unparseable reactant SMILES. Treat such routes as
        # zero-signal candidates instead of crashing the family evaluation.
...
def reaction_graph_descriptors(reactants: str, product: str) -> np.ndarray:
    ...
    return np.concatenate((reactant_desc, product_desc, product_desc - reactant_desc)).astype(np.float32)
